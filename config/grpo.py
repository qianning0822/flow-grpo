...
def general_ocr_sd3_1gpu():
    ...
    config.resolution = 512
    # ======= 显存优化修改 1：降低 Batch Size 与每组图数 =======
    config.sample.train_batch_size = 4        # 从 8 降到 4，极大缓解显存压力
    config.sample.num_image_per_prompt = 4     # 从 8 降到 4，减少 GRPO 采样群组大小
    ...
    config.sample.test_batch_size = 8          # 测试 Batch Size 也砍半
    ...
    # ======= 显存优化修改 2：强制开启低精度与显存卸载 =======
    config.mixed_precision = "bf16"            # 比 fp16 更稳，且省显存
    config.activation_checkpointing = True     # 开启重计算，用时间换空间（显著降低显存）
    config.fsdp_optimizer_offload = True       # 把优化器参数卸载到内存，再省几 GB 显存
